...
**Note: Remember to add the layer** 
...
   <summary>Answer</summary>
	...
	- `DISTRO_CODENAME = "unlocking transitions"`
	
   </details>


12. Now change  the `DISTRO` variable in `local.conf` to point to afry distro.
    Build a new image using `bitbake myimagetest` and run qemu.

	once inside try to run `cat /etc/issue` and see AFRY relfected there!


	## Advanced
	* Try running setting DISTRO variable to `poky-tiny` or `poky-bleeding` and see what happens