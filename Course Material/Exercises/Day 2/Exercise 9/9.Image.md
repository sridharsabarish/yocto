...
1. Start at `~/git/poky/meta-exercises`.
2. Create a folder called `recipes-images`, another folder called `myimagetest` inside it. 
   create a new blank recipe called `myimagetest.bb`, remember to place the `myimagetest.bb` under `myimagetest` folder!
...
   You can use `IMAGE_FSTYPES` metadata to accomplish this!
   ...
   <details>
   <summary>Answer</summary>
   Set IMAGE_FSTYPES+="ext4 ext3 wic"
</details>
  
  
5. Now add the line that sets the size of root file system to 200MB.

 <details>
   <summary>Answer</summary>
   IMAGE_ROOTFS_SIZE = "204857"
</details>

 Run `bitbake myimagetest` to see if these changes are reflected.
Identify the size of the root file ssytem in `/home/yocto/git/poky/build/tmp/deploy/images/qemux86-64/`
...
- `runqemu ~/git/poky/build/tmp/deploy/images/qemux86-64/myimagetest-qemux86-64.rootfs.ext4  nographic`

- **Note: Remember to remove the '`'` when you copy!!**
8. Once QEMU boots up, check if you can use the command `strace` inside it?


9. What happens when you type `perl`? can you use it? 

Add support for `perl` and build again and run qemu!
 <details>
   <summary>Answer</summary>
  IMAGE_INSTALL += "perl"
</details>
...
can you run nano in this image?


10 . Now Add support for `openssh` and `nano` and build again and run qemu!

 <details>
   <summary>Answer</summary>
  IMAGE_INSTALL += "openssh nano"
</details>


You will notice some ssh is starting up when you runqemu and that nano command is also available.
...
### Exploring Package Groups
 1.  Go to `~/git/poky/meta/recipes-extended/packagegroups`
...
`packagegroup-core-full-cmdline-multiuser `
 ...
 1. Can you find how to add support for crontab? 
