...
Now open the packages1 folder, you will see several recipes defined here.
...
3.  Now, take the `packages1_1.10` and add the following on top `PR="2.0"` and try to build now. Do you think this will get built now?
...
4. Now add `PE= 3` in `packages1_1.10.bb` and try building it. What do you notice?
   <details>
   <summary>Answer</summary>
...
7. Go to the `packages2` folder and try to add a dependency to a recipe called `magic`  and try to build this recipe. Do you see something strange?
...
9. Go to `packages3` folder and find the recipe packages3.bb.
 Rename this recipe to `magic.bb` and now run `bitbake packages2`, what do you see now? 
...
Packages can be generated for different formats like rpm, deb, ipk and so forth. Local.conf has a good reference on how to enable support. Please open `~/git/poky/build/conf/local.conf`.
...
**Note: There is a package  which corrupted during the VM creation. Please clean it using `bitbake -c cleanall tictactoe` before running the package-index command below**
