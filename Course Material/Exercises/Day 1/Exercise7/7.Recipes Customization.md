...
* Go to `~/git/poky/meta-exercises/recipes-extendrecipe/firstrec`
2.  Create an append  file for this recipe.  How should it be named?
   <details>
   <summary>Answer</summary>
   `firstrec.bbappend`
</details>
  3. Inside the append file `firstrec.bbappend` modify the values of the following variables.
* Set `PR `to `r12`
* Set `PE` to `4`
* Update the value for `CAT1`, `CAT2` and `CAT3` to as you feel like.

4. Now try to run `bitbake -e firstrec | grep  "^CAT1"` , similarly for other variables to check the new values.
   <details>
   <summary>Answer</summary>
  You will now see that the append file overwrote the normal recipe.
</details>

## Part 2: Applying a patch!
 ...
 * Open `~/git/poky/meta-exercises/recipes-extendrecipe`
...
16. Now try to commenting line 18 (the one which says bug) and uncomment line 20 (one with recipe65), what happens now?
 ...
 * Try commenting and uncommenting the "PACKAGECONFIG ??" variable.
...
# Advanced/Optional - Adding license to a recipe
* Note: if this is not present in your folder, consider downloading from the folder enclosed.

* Go to `recipes-extendrecipe/yoctocourse`, here there is a recipe file `yoctocourse.bbx`, rename it to `yoctocourse.bb`. This files is incorrect and does't have a license. if you try to build it you will face a QA Error.
  
*  Luckily we have the git repo cloned in `~/Downloads/yocto`. Go there and find the `LICENSE` file.
* Read the first line and identify the license type and run `md5sum filename` for example `md5sum LICENSE` and copy this to the `LIC_FILE_CHKSUM` variable.

We can find most licenses under `~git/poky/meta/files/common-licenses` also referred to as `COMMON_LICENSE_DIR`
