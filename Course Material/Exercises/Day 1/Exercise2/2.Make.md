...
- Gain a refresher for the syntax of make file
- Learn to automate the bitbake processes
...
* The exercises files are available in the `Exercise2` folder in the repo, for the sake of simplicity these have been moved to The deskop in the VM.

*  Go to `~/Desktop/Yocto_Course/Exercise/Exercise2`, exercise files are present here.

*  Explore the question below by referring to the `q1` folder
...
Target 2: Just populate info to clean the build (Don't actually clean the build)
...
Optional: open firefox and check localhost:8000 to see if toaster is running.
...
## Optional Example : Building a Kernel module 
...
`yocto-driver.c` has some code commented out, the comments provide some clues on how to rectify it. Try and fix it.
