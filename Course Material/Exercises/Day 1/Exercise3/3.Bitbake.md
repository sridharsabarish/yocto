...
   <summary>Answer</summary>
	...
	- With *E* the trick is to pay attention, the *C* doesn't have a `$`,so it is not expanded. The output would be D{C}.
   </details>
...
    FIKA ?= "Pain au chocolat"
```
   
   **Question:**  `What are the final values of CANDY, FIKA and DRINK?`
...
   <details>
   <summary>Answer</summary>
  A default value is assigned only when it is never assigned. Since there is an assignment,
  CANDY is assigned a value `"M&M"`
  DRINK takes on the first value which is "COFFEE"
  FIKA is of course "Kanelbulle" as assignment has higher preference than default values.
  
   </details>
...
   <details>
   <summary>Answer</summary>
	   - Weak defaults can replace previous weak default, so `SUMMER` is now 'AUGUST'
	   - WINTER is set as DECEMBER initially and stays the same.
...
  **Question** : Which Variables are the storing the same values?

   <details>
   <summary>hint</summary>
    How does override syntax work for appends and prepends? 
</details>
...
   <details>
   <summary>hint</summary>
	Is the override set in the OVERRIDES variable?
...
## Part 4 : Optional/Advanced - Cracking the Code 
